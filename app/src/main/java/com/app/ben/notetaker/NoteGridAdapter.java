...
class NoteGridAdapter extends ArrayAdapter<Note> implements Filterable {
    List<Note> objects;
    private List<Note> originalList = new ArrayList<>();
    Filter filter;
    private static final int WRAP_CONTENT_LENGTH = 5;

    public NoteGridAdapter(Context context, int resource, List<Note> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null) {
            convertView = LayoutInflater.from(getContext())
                    .inflate(R.layout.grid_component, parent, false);
        }
